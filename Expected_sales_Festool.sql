...
-- add product id, product name, product price, sales class, average sales in stock, total sales and stock magento
-- per product
...
with t1 as (SELECT  DISTINCT(p.product_id)                               AS PRODUCT_ID, 
                    ...
                    -- average sales in stock
                    ROUND(pc.mean,2)                                     AS AVG_DAILY_SALES_WHILE_IN_STOCK,
        
                    p.stock_magento                                      AS STOCK_MAGENTO,

                    -- the code is used for purchasing items for 160 days take it as the baseline to purchase
...
t5 as (SELECT *,
       ...
       -- when the stock magento >0 then subtruct CONFIRMED, BACKORDER and STOCK_MAGENTO from the baseline
       ...
       CASE WHEN STOCK_MAGENTO >= 0 AND  (STOCK_MAGENTO + CONFIRMED+BACKORDER) < PRIMARY_QTY      THEN PRIMARY_QTY-(CONFIRMED+BACKORDER+STOCK_MAGENTO)
          ...
          -- when the stock magento <0 then subtruct CONFIRMED, BACKORDER and add STOCK_MAGENTO to the baseline as it needs to be purchased
...
-- exclude EOL products and products have order qty <0 that may arise for products only having rma  items
